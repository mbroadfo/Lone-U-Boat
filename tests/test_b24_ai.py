...
class MockDice:
    ...
    def set_roll_sequence(self, rolls):
    ...
    def roll_2d6(self):
        """Return next roll in sequence (simulates 2d6)."""
        if self.roll_index < len(self.roll_sequence):
            roll = self.roll_sequence[self.roll_index]
            self.roll_index += 1
            return roll
...
def create_test_hex_grid():
    ...
    grid.mission_hexes = mission_hexes  # Store for is_valid_hex checks
    return grid
...
def test_b24_moves_two_hexes():
    ...
    messages, new_dl, remove = ai._activate_aircraft(aircraft, u_boat, detection_level=0)
...
def test_multiple_b24s_processed():
    ...
    messages, new_dl = ai.execute_b24_phase(aircraft_list, u_boat, detection_level=0)
...
def test_b24_no_turn_at_dl_1():
    ...
    messages, new_dl, remove = ai._activate_aircraft(aircraft, u_boat, detection_level=1)
...
def test_b24_turns_toward_uboat_at_dl_2():
    ...
    messages, new_dl, remove = ai._activate_aircraft(aircraft, u_boat, detection_level=2)
...
def test_b24_turns_randomly_in_same_hex_if_facing_off_map():
    ...
    messages = []
    ai._turn_aircraft(aircraft, u_boat, messages)
...
def test_flak_defense_destroys_b24_on_8_plus():
    """Flak gun destroys B-24 on roll of 8+."""
    dice = MockDice()
    dice.set_roll_sequence([8])  # 2d6 result
    grid = create_test_hex_grid()
    ai = B24AI(dice, grid)
    ...
    # B-24 should be destroyed
    assert remove
    assert any("B-24 destroyed/scared off" in msg for msg in messages)


def test_flak_defense_with_lookout_bonus():
    """Flak gun with lookout needs 7+ instead of 8+."""
    dice = MockDice()
    dice.set_roll_sequence([7])  # 2d6 result
    grid = create_test_hex_grid()
    ai = B24AI(dice, grid)
    ...
    # B-24 should be destroyed (7+ with lookout)
    assert remove
    assert any("B-24 destroyed/scared off" in msg for msg in messages)
...
def test_b24_attack_hull_damage_on_1_2():
    ...
    # Should deal +2 hull damage
    assert u_boat.hull_damage == 2
    assert any("+2 Hull Damage" in msg for msg in messages)
...
def test_b24_attack_damage_chart_on_3_4():
    ...
    # Should use damage chart (result depends on chart roll)
    assert u_boat.hull_damage >= 0  # Some damage may occur
