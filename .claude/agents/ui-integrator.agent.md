...
description: >
  ...
  and per-change commits. Spawned by the orchestrator after design-validator
  returns APPROVED.
...
4. The specific method(s) you will extend (use Grep to find exact line numbers first)
</mandatory_reads>

<analysis_paralysis_guard>
If you make 5 or more consecutive Read/Grep/Glob calls without any Edit/Write/Bash action:
...
```
grep -n "def _draw_<component>" core/screens/unified_game.py
```

Read the existing method — understand its inputs, what surface it draws to, and what rects it stores.
...
Build the full implementation inside `_draw_<component>_new()`. Reference the spec in `ux-redesign.agent.md` for exact colors, data sources, and behavior.
