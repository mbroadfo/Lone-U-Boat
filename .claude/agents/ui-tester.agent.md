...
description: >
  ...
  HUMAN_NEEDED. Spawned by orchestrator after ui-integrator completes.
...
**Level 1 — Exists:**
```bash
grep -n "def _draw_<component>_new" core/screens/unified_game.py
```
Status: FOUND / MISSING

**Level 2 — Substantive (not a stub):**
```bash
grep -A 30 "def _draw_<component>_new" core/screens/unified_game.py
```
Check for:
- `pass` only → STUB
- `# TODO` only → STUB
...
```bash
grep -n "NEW_UI_" config/board_config.py
grep -n "_draw_<component>_new\|NEW_UI_" core/screens/unified_game.py
```
Check:
- Feature flag exists in config → FOUND / MISSING
- Call site has `if NEW_UI_X:` branch with new method → WIRED / ORPHANED
...
Run targeted grep checks against the spec. Do NOT run the game — use static analysis only.

**Die colors (if dice tray component):**
```bash
grep -n "150, 160, 175\|220, 80, 60\|180, 150, 90\|220, 200, 60" core/screens/unified_game.py
```
Expected: at least one match near `_draw_<component>_new`. Flag if missing.

**Data source (if dice tray component):**
```bash
grep -n "action_points\|last_ap_roll\|last_escort_roll" core/screens/unified_game.py | grep -A2 -B2 "_new"
```
...
```bash
grep -n "ap_to_dice\|maxed\|while.*> 6\|remaining -= 6" core/screens/unified_game.py
```
Expected: some form of maxed-first split logic present.

**Animation guard (if scramble animation):**
```bash
grep -n "get_ticks\|scramble_start\|sleep\|delay" core/screens/unified_game.py | grep -v "#"
```
Flag: any `sleep` or `delay` call in the new method → blocks render loop.

**Board untouched:**
```bash
grep -n "def _draw_game_board" core/screens/unified_game.py
git diff HEAD~1 -- core/screens/unified_game.py | grep "^[+-]" | grep "_draw_game_board\|HEX_GRID\|CALIBRATION\|offset_x\|offset_y"
```
Expected: no changes to board method or calibration constants.
...
```bash
grep -n "TODO\|FIXME\|pass$\|# stub\|not implemented" core/screens/unified_game.py | grep -i "_new\b"
```
